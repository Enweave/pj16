...
func _update_body_sprite() -> void:
    if _current_action == AnimationStateKeys.ACTION:
        var current_ability: FeatureBase = player_character.ability_inventory.get_current_ability()
        if current_ability != null:
            if current_ability.get_animation_string_name() != "":
                body_sprite.play(current_ability.get_animation_string_name())
        return
    else:
        body_sprite.play(
            "%s_%s" % [
